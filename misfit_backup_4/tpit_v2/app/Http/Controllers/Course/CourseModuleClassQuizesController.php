...
class CourseModuleClassQuizesController extends Controller
{
    public function all()
    {
        ...
        $query = CourseModuleClassQuizes::where('status', $status)->orderBy($orderBy, $orderByType);
    ...
    public function show($id)
    {
        ...
        $data = CourseModuleClassQuizes::where('id', $id)
            ->select($select)
            ->first();
        if ($data) {
    ...
    public function store()
    {
        ...
        $data = new CourseModuleClassQuizes();
    ...
    {
        $data = CourseModuleClassQuizes::find(request()->id);
        if(!$data){
            return response()->json([
                'err_message' => 'validation error',
    ...
    {
        $validator = Validator::make(request()->all(), [
            'id' => ['required','exists:course_module_class_quizes,id'],
        ]);
    ...
    public function bulk_import()
    {
        ...
        foreach (request()->data as $item) {
            ...
            $item = (object) $item;
            $check = CourseModuleClassQuizes::where('id',$item->id)->first();
            if(!$check){
                try {
                    CourseModuleClassQuizes::create((array) $item);
                } catch (\Throwable $th) {
                    return response()->json([
                        'err_message' => 'validation error',
