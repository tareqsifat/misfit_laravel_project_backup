        ...
        (function ($) {
            ...
            function getOrderList(){
                ...
                $.ajax({
                    ...
                    cache: false,
                    beforeSend: function () {
                        $('.order-list').addClass('skeleton mb-2');
                    },
                    complete:function(){
                        setTimeout(() => {
                            $('.order-list').removeClass('skeleton mb-2');
                        }, 500);
                    },
            ...
            function orderPlace(data) {
                ...
                if(existsElement.length){
                    ...
                    <ul class="trading-left__list flex-between skeleton order-list order-rate-${order.rate.replace('.','_')} " data-rate="${order.rate}" data-total-amount='${order.amount}' data-total="${order.total}">
            ...
            function generateOrderHtml(order) {
                ...
                return `
                    <ul class="trading-left__list flex-between  order-list skeleton mb-2 ps-0 order-rate-${order.rate.replace('.','_')}" data-rate="${order.rate}" data-total-amount='${order.total_amount}' data-total="${total}">
                        <li class="price__item">${showAmount(order.rate)}</li>
