...
class SiteController extends Controller
{
    ...
    public function ai_robot() {
        $pageTitle = "Ai robot";
        $user      = auth()->user();
        
        $ai_robot_status=AiTrading::select('ai_robot')->first();
        return view($this->activeTemplate . 'ai_robot', compact('pageTitle', 'user', 'ai_robot_status'));
    }
    
    public function ai_robot_update(Request $request) {
        $userId = $request->input('userId');
        if($request->has('ai_status')) {
            $ai_status = 1;
        }else {
            $ai_status = 0;
        }
        ...
        $coin           = $pair->coin;
        $wallet = Wallet::where('user_id', $user->id)->where('currency_id', $coin->id)->first();
        $user->lock_amount = $wallet->balance;
        
        if ($user && $ai_status == 1) {
            
            $user->ai_robot = $ai_status;
            $user->lock_amount = $wallet->balance;
            $user->save();
            ...
            $notify[] = ['success', 'Ai trading updated successfully'];
            // $notify[] = ['success', 'Ai robot setting updated successfully'];
            return back()->withNotify($notify);
        }
        if($ai_status == 0) {
            $wallet->balance = $user->lock_amount; 
            $wallet->save();
            $user->lock_amount = 0;
            $user->save();
            $notify[] = ['success', 'Ai trading updated successfully'];
