...
function showPercent($amount, $decimal = 2, $separate = true, $exceptZeros = false)
{
    $separator = '';
    if ($separate) {
        $separator = ',';
...
{
    $text = '&copy; ' . date('Y') . ' <a href="' . route('home') . '" class="text--base"> ' . trans(gs('site_name')) . ' </a>. ' . trans('All Rights Reserved') . '';
    return $text;
}

function defaultCurrencyDataProvider(): object
{
