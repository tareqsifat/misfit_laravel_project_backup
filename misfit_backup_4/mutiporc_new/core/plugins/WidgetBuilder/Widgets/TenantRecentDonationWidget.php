...
class TenantRecentDonationWidget extends WidgetBase
{
    ...
    public function frontend_render()
    {
        ...
        $blog_posts = Donation::where(['status' => 1])->take($blog_items)->orderBy('id','desc')->get();

        $markup = '';
        foreach ($blog_posts as $post){

            $image = render_background_image_markup_by_attachment_id($post->image,'','thumb');
            $route = route('tenant.frontend.donation.single',$post->slug);
...
ITEM;
 ...
 return <<<HTML
     <div class="simplePresentCart2 mb-30 mt-5">
