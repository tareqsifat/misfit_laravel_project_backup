...
class MenuBuilderFrontendRender
{
    ...
    public function render_frrontend_panel_menu($id){
        ...
        $menu_data = json_decode($menu_details_from_db[0]->content ?? $menu_details_from_db->content); //Sohan edited this
        $this->page_id = 1;
    ...
    private function render_menu_item($menu_item, int $page_id, $default_lang = 'en_US')
    {
        ...
        if ($ptype === 'custom'){
            ...
            $output .= $this->get_anchor_markup(__($title),[
                'href' => !empty($menu_item->purl) ? str_replace('@url',url('/'),$menu_item->purl) :  url('/'),
            ...
            if (!empty($matches[0])){
                ...
                if ($dynamic_menu_type){
                    ...
                    if ($dynamic_menu_type['query'] === 'old_lang'){
                        ...
                        $item_details = $model->usingLocale($default_lang ?? default_lang())->where(['id' => $menu_item->pid,'status' => 1])->first();
                    }
                    ...
                    if(!empty($title)){
                        $output .= $this->get_anchor_markup($title,[
                            'href' => Route::has($dynamic_menu_type['route']) ? route($dynamic_menu_type['route'],$route_params) : '',
                            'target' => $menu_item->antarget ?? url('/'),
                        ],$menu_item->icon ?? '');
    ...
    private function get_anchor_markup(string $title,array $args, $icon = null)
    {
        ...
        return "\t\t".'<a '.$this->get_attribute_string($args).'class="menuArrow">'.$icon_markup.strip_tags($title).'</a>'."\n";
    }
}
