...
<div class="mail-container">
    ...
    <div class="main-content-wrap">
        <p>{{__('Hello')}}</p>
        @if($type == 'user')
        <p>{{__('A payment from')}} {{$data->name}} {{__('was successful. OrderID')}} #{{$data->id}} ,{{__('Package Name')}} : "{{$data->package_name}}" {{__('Paid Via')}} {{ucfirst(str_replace('_',' ',$data->package_gateway))}}</p>
        @else
            <p>{{__('You get payment from')}} {{$data->name}} {{__('For wedding order ID') .'#'}} {{$data->id}}, {{__('Package Name')}} : {{$data->package_name}} {{__('paid via')}} {{ucfirst(str_replace('_',' ',$data->package_gateway))}}</p>
        @endif
        <div class="price-wrap">{{amount_with_currency_symbol($data->package_price)}}</div>
        <table>
            <tr>
                <td>{{__('Order ID')}}</td>
                <td>#{{$data->id ?? ''}}</td>
