...
use Modules\GeneralSetting\Http\Controllers\Tenant\Admin\GeneralSettingController;
use Modules\AppearanceSetting\Http\Controllers\Tenant\Admin\AppearanceSettingController;
use Modules\PaymentSetting\Http\Controllers\Tenant\Admin\PaymentSettingController;
use Modules\PricePlan\Http\Controllers\Tenant\Admin\PricePlanController;
use Modules\Advertisement\Http\Controllers\Tenant\Admin\AdvertisementController;
use Modules\Faq\Http\Controllers\Tenant\Admin\FaqController;
use Modules\ExternalImport\Http\Controllers\Tenant\Admin\ExternalImportController;
use Modules\Integration\Http\Controllers\Tenant\Admin\IntegrationController;
use Modules\ImageGallery\Http\Controllers\Tenant\Admin\ImageGalleryController;
use Modules\Pages\Http\Controllers\Tenant\Admin\PagesController;
use Modules\MyPaymentLog\Http\Controllers\Tenant\Admin\MyPaymentLogController;
use Modules\Testamonia\Http\Controllers\Tenant\Admin\TestamoniaController;
use Modules\FormBuilder\Http\Controllers\Tenant\Admin\FormBuilderController;
use Modules\Brands\Http\Controllers\Tenant\Admin\BrandController;
use Modules\CustomDomain\Http\Controllers\Tenant\Admin\CustomDomainController;
use Modules\Staff\Http\Controllers\Tenant\Admin\StaffController;
...
----------------------------*/
   ...
   -------------------------------------------- */
    ...
    Route::group(['prefix' => 'gene', "as" => "admin."], function () {
        Route::controller(GeneralSettingController::class)->group(function () {
            Route::get('/', 'index')->name('pageSetting');
            Route::get('basicSetting', 'basicSetting')->name('basicSetting');
            Route::get('colorSetting', 'colorSetting')->name('colorSetting');
            Route::get('typography', 'typography')->name('typography');
            Route::get('seoSetting', 'seoSetting')->name('seoSetting');
            Route::get('siteIdentity', 'siteIdentity')->name('siteIdentity');
            Route::get('emailSetting', 'emailSetting')->name('emailSetting');
            Route::get('thirdParty', 'thirdParty')->name('thirdParty');
            Route::get('css', 'css')->name('css');
            Route::get('js', 'js')->name('js');
        });
    });
    Route::group(['prefix' => 'appearance', "as" => "admin."], function () {
        Route::controller(AppearanceSettingController::class)->group(function () {
            Route::get('404setting', 'setting')->name('404setting');
            Route::get('maintainanceSetting', 'maintainanceSetting')->name('maintainanceSetting');
            Route::get('menuManage', 'menuManage')->name('menuManage');
            Route::get('topBarSetting', 'topBarSetting')->name('topBarSetting');
            Route::get('otherSetting', 'otherSetting')->name('otherSetting');
        });
    });
    Route::group(['prefix' => 'paymentSetting', "as" => "admin."], function () {
        Route::controller(PaymentSettingController::class)->group(function () {
            Route::get('authorizeNet', 'authorizeNet')->name('authorizeNet');
            Route::get('bankTransfer', 'bankTransfer')->name('bankTransfer');
            Route::get('billPlz', 'billPlz')->name('billPlz');
            Route::get('cashFree', 'cashFree')->name('cashFree');
            Route::get('cinetPay', 'cinetPay')->name('cinetPay');
            Route::get('flutterWave', 'flutterWave')->name('flutterWave');
            Route::get('instaMojo', 'instaMojo')->name('instaMojo');
            Route::get('kinetic', 'kinetic')->name('kinetic');
            Route::get('manualPayment', 'manualPayment')->name('manualPayment');
            Route::get('marCadoPago', 'marCadoPago')->name('marCadoPago');
            Route::get('midTrans', 'midTrans')->name('midTrans');
            Route::get('pagali', 'pagali')->name('pagali');
            Route::get('payFast', 'payFast')->name('payFast');
            Route::get('paypal', 'paypal')->name('paypal');
            Route::get('payStack', 'payStack')->name('payStack');
            Route::get('payTabs', 'payTabs')->name('payTabs');
            Route::get('paytm', 'paytm')->name('paytm');
            Route::get('razorPay', 'razorPay')->name('razorPay');
            Route::get('sitesWay', 'sitesWay')->name('sitesWay');
            Route::get('squareUp', 'squareUp')->name('squareUp');
            Route::get('stripe', 'stripe')->name('stripe');
            Route::get('toyyibPay', 'toyyibPay')->name('toyyibPay');
            Route::get('zitoPay', 'zitoPay')->name('zitoPay');
            Route::get('paymentSetting', 'paymentSetting')->name('paymentSetting');
            Route::get('mollie', 'mollie')->name('mollie');
        });
    });
    Route::group(['prefix' => 'staff', "as" => "admin."], function () {
        Route::controller(StaffController::class)->group(function () {
            Route::get('addNewStaff', 'addNewStaff')->name('addNewStaff');
            Route::get('staff', 'staff')->name('allStaff');
            Route::get('staffRole', 'staffRole')->name('staffRole');
     
        });
    });
    Route::group(['prefix' => 'brands', "as" => "admin."], function () {
    ...
    Route::group(['prefix' => 'customDomain', "as" => "admin."], function () {
        Route::controller(CustomDomainController::class)->group(function () {
            Route::get('customDomains', 'customDomains')->name('customDomains');
     
        });
    });
    Route::group(['prefix' => 'pages', "as" => "admin."], function () {
    ...
    Route::group(['prefix' => 'pricePlan', "as" => "admin."], function () {
        Route::controller(PricePlanController::class)->group(function () {
            Route::get('allPlan', 'allPlan')->name('allPlan');
            Route::get('planPaymentLogs', 'planPaymentLogs')->name('planPaymentLogs');
     
        });
    });
    Route::group(['prefix' => 'advertisement', "as" => "admin."], function () {
        Route::controller(AdvertisementController::class)->group(function () {
            Route::get('allAdvertisement', 'allAdvertisement')->name('allAdvertisement');
            Route::get('addAdvertisement', 'addAdvertisement')->name('addAdvertisement');
     
        });
    });
    Route::group(['prefix' => 'faq', "as" => "admin."], function () {
        Route::controller(FaqController::class)->group(function () {
            Route::get('allFaqs', 'allFaqs')->name('allFaqs');
            Route::get('categoryFaqs', 'categoryFaqs')->name('categoryFaqs');
     
        });
    });
    Route::group(['prefix' => 'ImageGallery', "as" => "admin."], function () {
        Route::controller(ImageGalleryController::class)->group(function () {
            Route::get('allImage', 'allImage')->name('allImage');
            Route::get('categoryImage', 'categoryImage')->name('categoryImage');
     
        });
    });
    Route::group(['prefix' => 'externalImport', "as" => "admin."], function () {
        Route::controller(ExternalImportController::class)->group(function () {
            Route::get('aliExpress', 'aliExpress')->name('aliExpress');
     
        });
    });
    Route::group(['prefix' => 'integration', "as" => "admin."], function () {
        Route::controller(IntegrationController::class)->group(function () {
            Route::get('product', 'product')->name('product');
     
        });
    });
    Route::group(['prefix' => 'testamonia', "as" => "admin."], function () {
        Route::controller(TestamoniaController::class)->group(function () {
            Route::get('testmonials', 'testmonials')->name('testmonials');     
        });
    });
    Route::group(['prefix' => 'formBuilder', "as" => "admin."], function () {
        Route::controller(FormBuilderController::class)->group(function () {
            Route::get('customForm', 'customForm')->name('customForm');     
            Route::get('formSubmission', 'formSubmission')->name('formSubmission');     
        });
    });
    Route::group(['prefix' => 'myPaymentLog', "as" => "admin."], function () {
        Route::controller(MyPaymentLogController::class)->group(function () {
            Route::get('mypaymentlog', 'mypaymentlog')->name('mypaymentlog');     
        });
    });
