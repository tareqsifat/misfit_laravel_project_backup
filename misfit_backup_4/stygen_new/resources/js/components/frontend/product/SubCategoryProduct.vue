                ...
                <div class="row">
                    <div class="col-12">
                        <div class="breadcrumb-content text-center">
                            <ul>
                                <li><router-link :to="{name: 'landing'}">Home </router-link><router-link :to="{name: 'subCategoryProduct', params: {catSlug:category_slug}}">/ Category</router-link></li>
                                <li class="active">{{ category_slug_name }}</li>
                            </ul>
                        </div>
                    </div>
                </div>
    ...
    export default {
        ...
        metaInfo: {
            ...
            meta: [
                ...
                { name: 'title', content: localStorage.getItem('meta_title') },
        ...
        methods:{
            ...
            productList(){
                let payload = {'page': this.currentPage, 'cat_slug': this.$route.params.catSlug}
            ...
            getCategorySlug(){
                axios.post('/category-product-slug', {'cat_slug': this.$route.params.catSlug})
                    .then((result) => {
                        this.category_slug_name = result.data.category_name
                        this.category_slug = result.data.cat_slug
                    }).catch((error) => {
                });
            },
            addToCart(product){
            ...
            productSorting: function productSorting(event) {
                ...
                let payload = {'page': this.currentPage, 'keyword': '', 'sort_value': this.sortValue, 'category_slug': this.category_slug}
                this.$store.dispatch('product/categoryProductSort', payload)
            },
            productCatName() {
                axios.post('/product-category-name', {'category_slug': this.$route.params.catSlug})
                    .then((result) => {
                        this.meta_title = result.data.meta_title
                        this.meta_description = result.data.meta_description
