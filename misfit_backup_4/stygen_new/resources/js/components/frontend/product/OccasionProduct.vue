                ...
                <div class="row">
                    <div class="col-12">
                        <div class="breadcrumb-content text-center">
                            <ul>
                                <li><router-link :to="{name: 'landing'}">Home </router-link><router-link :to="{name: 'occasionProduct', params: {occasionSlug: occasion_slug}}">/ Gift by Occasion</router-link></li>
                                <li class="active">{{ occasion_slug_name }}</li>
                            </ul>
                        </div>
                    </div>
                </div>
            ...
            <div :class="{ container: isActive }">
                <div class="row">
                    <div class="col-sm-12 col-md-12 col-lg-12">
                        <div class="">
                            <carousel v-if="sliders.length > 0" :autoplay="true" :loop="false" :center="true" :nav="false" :items="1" :margin="5" :autoplayHoverPause="true">
                                <img v-for="(slider, index) in sliders" :key="index" :src="`/assets/uploads/slider/${slider.image}`" :alt="slider.title">
                            </carousel>
                        </div>
                    </div>
                </div>
                            ...
                            <!-- <div class="toolbar-short-area d-md-flex align-items-center">
                                <div class="toolbar-shorter">
                                    ...
                                    <select class="productSorting">
                                        <option value="" v-for="pro_category in occasion_categories" :key="pro_category.id">{{ pro_category.category_name }}</option>
                                    </select>
                                </div>
                            </div> -->

                            <h4 class="mb-2">Filter by Category</h4>
                            <div class="container-fluid filter mb-4">
                                <div class="row">
                                    <carousel :autoplay="true" :loop="true" :center="true" :dots="true" :nav="true" :items="6" :margin="5" :autoplayHoverPause="true">
                                        <span v-for="pro_category in occasion_categories" :key="pro_category.id">
                                            <input type="radio" :id="pro_category.id" class="ocassion_cat" :value="pro_category.id" v-model="ocassion_cat">
                                            <label :for="pro_category.id" class="btn btn-sm border">{{ pro_category.category_name }}</label>
                                        </span>
                                    </carousel>
                                </div>
                            </div>

                            <!-- <button type="button" class="btn btn-outline-primary" data-bs-toggle="button" autocomplete="off" ></button> -->
...
<script>
    ...
    export default {
        ...
        methods:{
            ...
            getSingleProductSlider(){
                let payload = {'occasion_slug': this.$route.params.occasionSlug}
                this.$store.dispatch('slider/getSingleProductSlider', payload);
            },
            productList(){
                let payload = {'page': this.currentPage, 'occasion_slug': this.$route.params.occasionSlug, 'ocassion_cat': this.ocassion_cat}
            ...
            paginationChange(){
                this.scrollToTop();
                let payload = {'page': this.currentPage, 'occasion_slug': this.$route.params.occasionSlug}
                this.$store.dispatch('product/getOccasionProducts', payload)
            },
            scrollToTop() {
            ...
            Ocassion_product_sort() {
                let payload = {'page': this.currentPage, 'keyword': '','occasion_slug': this.occasion_slug, 'ocassion_cat': this.ocassion_cat}
                this.$store.dispatch('product/occassionProductSortByCat', payload)
            },
            productOccassionName() {
                axios.post('/product-occassion-name', {'occasionSlug': this.$route.params.occasionSlug})
                    .then((result) => {
                        console.log(result);
            ...
            ocassion_categories() {
                axios.post('/ocassion-product-category', {'occasionSlug': this.$route.params.occasionSlug})
                    .then((result) => {
                        this.occasion_categories = result.data.ocassion_categories.slice().reverse()
                        // this.occasion_slug = result.data.occasion_slug
                    }).catch((error) => {
...
input.ocassion_cat:checked + label {
    background-color: #5e2e87;
    color: white;
