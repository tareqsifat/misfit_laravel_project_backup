...
<div class="row first-section">
    <div class="column f1">
        {{-- @if(!empty($companyInfo->image))
            <img class="company-logo" src="{{ asset('assets/uploads/seller/'.$companyInfo->image) }}" alt="" style="width:120px !important; height:auto !important;">
        @else
...
<div class="row third-section">
    <div class="column sec1">
        @if(isset($order->customer))
            <p class="fontSize">{{ $order->customer['customer_name'] }}</p>
            <?php
            $cus_address = $order->customer['customer_address'];
            $length = strlen($cus_address);
            $cus_final_address = substr($cus_address, 0, 39);
            $cus_final_address1 = substr($cus_address, 39, $length);
            ?>
            <p class="fontSize"><?php echo $cus_final_address; ?></p>
            <p class="fontSize"><?php echo $cus_final_address1; ?></p>
            <p class="fontSize">{{ $order->customer['customer_phone'] }}</p>
            <p class="fontSize">{{ $order->customer['customer_email'] }}</p>
        @endif
    </div>

    <div class="column sec3" style="margin-top: 10px !important;">
...
<!--Start Fourth Section-->
<div class="row fourth-section" style="padding-top: 7% !important;">
    <div class="column">
        <table>
            <tr>
                <th class="product-name">Product</th>
                <th class="product-sku">SKU</th>
                @if($order->order_attributes->count() > 0)
            ...
            @foreach($order_details as $order_detail)
                <tr>
                    ...
                    @php
                        $netTotal += $order_detail->quantity * $order_detail->price;
                    @endphp
                </tr>
            @endforeach
        </table>
        <div class="row calculation-section" style="padding-top:1% !important;">
            ...
            <div class="column col3">
                @if(isset($netTotal))
                    <p class="totalVal fontSize"> {{number_format($netTotal, 2)}}</p>
                @else
                    <p class="totalVal fontSize"> 0</p>
                @endif
                <hr class="custom-hr7">
                @if(isset($discount_amount))
                    <p class="discountVal fontSize"> {{number_format($discount_amount, 2)}} </p>
                @else
                    <p class="discountVal fontSize"> 0</p>
                @endif
                <hr class="custom-hr3">
                @if(isset($net_receivable))
                    <p class="netReceivableVal fontSize"> {{number_format($net_receivable, 2)}} </p>
                @else
