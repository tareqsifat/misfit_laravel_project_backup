                            ...
                            @forelse($providers as $provider)
                                <tr>
                                    <td>
                                        <div class="user">
                                            <div class="thumb">
                                                <img src="{{ getImage(getFilePath('currency_data_provider') .'/coinmarketcap.png',getFileSize('currency_data_provider')) }}">
                                            </div>
                                    ...
                                    <td>
                                        ...
                                        data-question="@lang('Are you sure to disable this currency data provider')?">
                                                <i class="la la-check"></i>@lang('Yes')
                                        </button>
                                        @else
                                        <span>@lang('Yes')</span>
                                        @endif
                                    ...
                                    <td>
                                        <div class="button--group">
                                            ...
                                            <button type="button" class="btn btn-sm btn-outline--dark ms-1 mb-2 helpBtn"
                                                    data-description="{{ __($provider->description) }}"
                                                    data-support="{{ __($provider->support) }}" data-name="{{ $provider->name }}">
    ...
    <script>
        (function ($) {
            ...
            $(document).on('click', '.helpBtn',function () {
                ...
                let html="";
                 if(name.toLowerCase() == "coinmarketcap"){
                    html+=`
                    <ul class="list-group list-group-flush">
                        <li class="list-group-item">@lang('Go to the CoinMarketCap website') <a href="https://coinmarketcap.com/api" target="_blank">https://coinmarketcap.com/api</a></li>
                    ...
                    modal.find('.modal-title').html(`@lang('Coinmarketcap api key setting instruction')`);
                }
                modal.find('.modal-body').html(html);
                modal.modal('show');
            });
