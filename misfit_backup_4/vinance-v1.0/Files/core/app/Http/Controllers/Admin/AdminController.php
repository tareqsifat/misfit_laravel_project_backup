...
class AdminController extends Controller
{
    ...
    public function dashboard()
    {
        ...
        $withdraw                             = Withdrawal::with('withdrawCurrency')->where('status', '!=', Status::PAYMENT_REJECT)->groupBy('currency');
        $widget['withdraw']['list']           = (clone $withdraw)->selectRaw('*,SUM(amount) as total_amount')->orderBy('total_amount', 'DESC')->take(6)->get();
        $widget['withdraw']['currency_count'] = (clone $deposit)->selectRaw('*,count(*) as count')->orderBy('count', 'DESC')->get()->plucK('count');
        $widget['withdraw']['currency_symbol']  = (clone $deposit)->selectRaw('*,count(*) as count')->orderBy('count', 'DESC')->get()->plucK('currency.symbol');
